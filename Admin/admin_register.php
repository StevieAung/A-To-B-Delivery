...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create') {
    ...
    if ($password !== $confirm) {
        ...
        if ($res->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO admins (name, email, role, password_hash) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $name, $email, $role, $hash);

            if ($stmt->execute()) {
                $alert = "<div class='alert alert-success mt-3 p-2 text-center'>✅ Admin created successfully!<br><strong>$email</strong> ($role)</div>";
            } else {
                $alert = "<div class='alert alert-danger mt-3 p-2 text-center'>Database error. Please try again.</div>";
            }
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    ...
    if ($deleteId === $_SESSION['admin_id']) {
        ...
        if ($stmt->execute()) {
            $alert = "<div class='alert alert-success mt-3 p-2 text-center'>🗑 Admin account deleted successfully.</div>";
        } else {
            $alert = "<div class='alert alert-danger mt-3 p-2 text-center'>Failed to delete admin. Please try again.</div>";
        }
    }
}
?>
...
<h4 class="text-primary mb-4">🧑‍💼 Super Admin – Manage Administrators</h4>
...
<div class="row g-4">
  ...
  <div class="col-lg-6">
    <div class="card p-4">
      <h5 class="text-primary mb-3">📋 Existing Admins</h5>
      <div class="table-responsive">
        <table class="table table-hover align-middle">
          ...
          <tbody>
            <?php
            $admins = $conn->query("SELECT admin_id, name, email, role FROM admins ORDER BY created_at DESC");
            ...
            <tr>
              ...
              <td>
                ...
                <form method="POST" action="admin_register.php" onsubmit="return confirm('Are you sure you want to delete this admin?');" class="d-inline">
                  <?php csrf_input(); ?>
